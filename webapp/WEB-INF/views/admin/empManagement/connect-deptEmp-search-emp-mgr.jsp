	...
	$(document).on("click", ".select-dept", function(event) {
		...
		$.ajax({
			...
			success : function(response) {
				...
				$(opener.document).find('#dept').attr("data-lang-biz",response.data);
			},
			error : function(xhr, status, e) {
				console.error(status + ":" + e);
			}
		...
		$(opener.document).find('#dept').val(selectDeptName);
		$(opener.document).find('#dept').attr("data-lang",selectDeptSeq);
